...
import { del as deleteBlob, head, list, put } from "@vercel/blob";
...
const remoteMediaIndexPath = "nexa-meta/media-library.json";
const remoteMediaRecordsPrefix = "nexa-meta/media-records/";
const remoteAlbumsIndexPath = "nexa-meta/albums.json";
...
const allowedArtworkTypes = new Set([
  "image/jpeg",
  "image/png",
  "image/webp",
  "image/gif"
...
const stripeCheckoutPlans = {
  "jesus-track-vocal": {
    name: "JESUS CHRIST: THE LAMB OF ROYAL MERCIES - Vocal Track MP3/WAV",
    amount: 134,
    mode: "payment",
    entitlement: "track"
  },
  "jesus-track-instrumental": {
    name: "JESUS CHRIST: THE LAMB OF ROYAL MERCIES - Instrumental Track MP3/WAV",
    amount: 99,
    mode: "payment",
    entitlement: "track"
  },
  "jesus-album": {
    name: "JESUS CHRIST: THE LAMB OF ROYAL MERCIES - Album MP3/WAV",
    amount: 999,
    mode: "payment",
    entitlement: "album"
  },
  "live-streaming-monthly": {
    name: "NEXAMusic™ Studios Live Streaming Pass",
    amount: 999,
    mode: "subscription",
...
function canUseBlobMetadata() {
  return Boolean(process.env.VERCEL && process.env.BLOB_READ_WRITE_TOKEN);
}

async function loadRemoteMediaIndex({ recoverOrphans = true } = {}) {
  if (!canUseBlobMetadata()) return;
  let loadedRemote = false;
  try {
    const blob = await head(remoteMediaIndexPath);
    const res = await fetch(`${blob.url}?t=${Date.now()}`, { cache: "no-store" });
    if (!res.ok) throw new Error(`Remote media index returned ${res.status}`);
    const data = await res.json();
    if (Array.isArray(data)) {
      writeJsonFile(mediaIndexFile, data);
      loadedRemote = true;
    }
  } catch (error) {
    if (!String(error?.message || "").toLowerCase().includes("not found")) {
      console.warn("Media index could not be loaded from Blob:", error?.message || error);
    }
  }
  const recordItems = await loadRemoteMediaRecords();
  if (recordItems.length) {
    const merged = dedupeMediaByUrl([...readJsonFile(mediaIndexFile), ...recordItems]);
    writeJsonFile(mediaIndexFile, merged);
    loadedRemote = true;
  }
  if (recoverOrphans) await recoverOrphanBlobMedia(loadedRemote);
}

async function saveRemoteMediaIndex(data) {
  if (!canUseBlobMetadata()) return;
  await put(remoteMediaIndexPath, JSON.stringify(data, null, 2), {
    access: "public",
    allowOverwrite: true,
...
async function loadRemoteMediaRecords() {
  if (!canUseBlobMetadata()) return [];
  try {
    const result = await list({ prefix: remoteMediaRecordsPrefix, limit: 1000 });
    ...
    console.warn("Media record recovery skipped:", error?.message || error);
    return [];
  }
}

async function saveRemoteMediaRecord(item) {
  if (!canUseBlobMetadata() || !item?.id) return;
  await put(`${remoteMediaRecordsPrefix}${item.id}.json`, JSON.stringify(item, null, 2), {
    access: "public",
    allowOverwrite: true,
    contentType: "application/json; charset=utf-8",
    cacheControlMaxAge: 60
...
async function loadRemoteAlbumsIndex() {
  if (!canUseBlobMetadata()) return;
  try {
    const blob = await head(remoteAlbumsIndexPath);
    const res = await fetch(`${blob.url}?t=${Date.now()}`, { cache: "no-store" });
    if (!res.ok) throw new Error(`Remote albums index returned ${res.status}`);
    const data = await res.json();
    if (Array.isArray(data)) writeJsonFile(albumsFile, mergeById(data, seedAlbums));
  } catch (error) {
    if (!String(error?.message || "").toLowerCase().includes("not found")) {
      console.warn("Albums index could not be loaded from Blob:", error?.message || error);
    }
  }
}

async function saveRemoteAlbumsIndex(data) {
  if (!canUseBlobMetadata()) return;
  await put(remoteAlbumsIndexPath, JSON.stringify(data, null, 2), {
    access: "public",
    allowOverwrite: true,
    contentType: "application/json; charset=utf-8",
    cacheControlMaxAge: 60
...
function mediaTitleFromPathname(pathname) {
  const parsed = path.parse(path.basename(pathname));
  return parsed.name
    .replace(/^\d{10,}-/, "")
    .replace(/[-_][a-zA-Z0-9_-]{20,}$/, "")
    .replace(/[-_]+/g, " ")
    .replace(/\s+/g, " ")
    .trim()
    .replace(/\b\w/g, (char) => char.toUpperCase()) || "Uploaded Media";
}

function trackNumberFromPathname(pathname) {
  const parsed = path.parse(path.basename(pathname));
  const match = parsed.name.replace(/^\d{10,}-/, "").match(/^track[-_\s]*(\d+)/i);
  return match ? match[1] : "";
}

function mimeTypeFromPathname(pathname) {
  const ext = path.extname(pathname).toLowerCase();
  return mimeTypes[ext]?.replace(/;.*$/, "") || "application/octet-stream";
}

function orphanMediaRecord(blob) {
  const mimeType = mimeTypeFromPathname(blob.pathname);
  const trackNumber = trackNumberFromPathname(blob.pathname);
  return {
    id: `blob-${crypto.createHash("sha1").update(blob.pathname).digest("hex").slice(0, 16)}`,
    createdAt: new Date(blob.uploadedAt || Date.now()).toISOString(),
    title: mediaTitleFromPathname(blob.pathname),
    artistId: "",
    artist: "NEXAMusic™ Studios",
    albumId: "",
    album: "Uploaded Media",
    genre: "",
    trackNumber,
    kind: mimeType.startsWith("video/") ? "video" : "full track",
    releaseStatus: "active",
    isSnippet: false,
    provider: "blob",
    youtubeId: "",
...
async function recoverOrphanBlobMedia(remoteLoaded) {
  try {
    const result = await list({ prefix: "nexa-media/", limit: 1000 });
    if (!result.blobs?.length) return;
    const media = readJsonFile(mediaIndexFile);
    const seenUrls = new Set(media.map((item) => item.url).filter(Boolean));
    const recovered = result.blobs
      .filter((blob) => !seenUrls.has(blob.url))
      .map(orphanMediaRecord);
    if (!recovered.length) return;
    const next = [...media, ...recovered];
    writeJsonFile(mediaIndexFile, next);
    if (remoteLoaded || media.length || recovered.length) await saveRemoteMediaIndex(next);
  } catch (error) {
    console.warn("Blob media recovery skipped:", error?.message || error);
  }
}

function recoveredBlobScore(item) {
  let score = 0;
  if (item.artistId) score += 4;
  if (item.albumId) score += 3;
  if (item.genre) score += 2;
  if (item.trackNumber) score += 1;
  if (item.artist && item.artist !== "NEXAMusic™ Studios") score += 2;
  if (item.album && item.album !== "Uploaded Media") score += 2;
  if (item.title && !String(item.id || "").startsWith("blob-")) score += 2;
  return score;
}

function dedupeMediaByUrl(items) {
  const byUrl = new Map();
  items.forEach((item) => {
    const key = item.url || item.id;
    const existing = byUrl.get(key);
    if (!existing || recoveredBlobScore(item) >= recoveredBlobScore(existing)) {
      byUrl.set(key, item);
    }
  });
...
function hasPurchasedDownload(user, track) {
  const purchases = user?.purchases || {};
  const trackIds = new Set(purchases.trackIds || []);
  const albumIds = new Set(purchases.albumIds || []);
  return Boolean(trackIds.has(track?.id) || albumIds.has(track?.albumId) || albumIds.has(sanitizeFilename(track?.album || "").replace(/\.[^.]+$/, "")));
}

function hasDownloadAccess(user, track) {
  return hasLiveStreamingSubscription(user) || hasPurchasedDownload(user, track);
}

function grantPurchase(users, userId, metadata = {}) {
  const index = users.findIndex((item) => item.id === userId || item.email === metadata.email);
  if (index === -1) return null;
  const plan = sanitizeText(metadata.plan);
  const now = new Date().toISOString();
  if (plan === "live-streaming-monthly") {
    users[index].subscription = {
      liveStreaming: true,
      plan,
      artistId: sanitizeText(metadata.artistId, "all-artists"),
      activeUntil: new Date(Date.now() + 31 * 24 * 60 * 60 * 1000).toISOString(),
      updatedAt: now
    };
  } else {
    const purchases = users[index].purchases || {};
    const trackIds = new Set(purchases.trackIds || []);
    const albumIds = new Set(purchases.albumIds || []);
    if (metadata.trackId) trackIds.add(sanitizeText(metadata.trackId));
    if (metadata.albumId) albumIds.add(sanitizeText(metadata.albumId));
...
function parseGoogleDriveUrl(urlValue) {
  const value = String(urlValue || "").trim();
  if (!value) return null;
  try {
    const parsed = new URL(value);
...
function mediaMimeFromFilename(filename, fallback = "") {
  const normalized = String(fallback || "").split(";")[0].trim().toLowerCase();
  if (allowedMediaTypes.has(normalized)) return normalized;
  const inferred = mimeTypeFromPathname(filename);
  return allowedMediaTypes.has(inferred) ? inferred : normalized;
}

async function importGoogleDriveMedia(drive, fallbackTitle = "drive-media") {
  if (!process.env.BLOB_READ_WRITE_TOKEN) {
    throw new Error("Vercel Blob storage is required for Google Drive imports. Add BLOB_READ_WRITE_TOKEN in Vercel.");
  }
  const res = await fetch(drive.downloadUrl, { redirect: "follow" });
  if (!res.ok) throw new Error(`Google Drive import failed with status ${res.status}. Make sure the file is shared publicly.`);

  const contentType = (res.headers.get("content-type") || "").split(";")[0].trim().toLowerCase();
  if (contentType.includes("html") || contentType.includes("text/plain")) {
    throw new Error("Google Drive returned a web page instead of media. Set the Drive file sharing to 'Anyone with the link can view'.");
  }

  const dispositionName = filenameFromContentDisposition(res.headers.get("content-disposition"));
  const filename = sanitizeFilename(dispositionName || `${fallbackTitle}${contentType.startsWith("video/") ? ".mp4" : ".mp3"}`);
  const mimeType = mediaMimeFromFilename(filename, contentType);
  if (!allowedMediaTypes.has(mimeType)) {
    throw new Error("Google Drive file must be an audio or video file.");
  }

  const buffer = Buffer.from(await res.arrayBuffer());
  if (!buffer.length) throw new Error("Google Drive file was empty.");
  if (buffer.length > 100 * 1024 * 1024) throw new Error("Google Drive imports must be 100MB or smaller.");

  const blob = await put(`nexa-media/${Date.now()}-${filename}`, buffer, {
    access: "public",
...
function uploadedTracks() {
  ...
  return dedupeMediaByUrl(readJsonFile(mediaIndexFile))
...
function getDashboardData() {
  ...
  const tracks = [
    ...builtInTracks.map((item) => ({ ...item, isUpload: false })),
    ...uploads.map((item) => ({
      id: item.id,
      title: item.title,
...
function getPublicMediaLibrary() {
  ...
  return [...builtInTracks.map((item) => ({ ...item, isUpload: false })), ...uploads]
    .map((item) => ({
      ...item,
      size: item.size || 0,
...
function filePathForTrackFormat(track, format = "mp3") {
  const basePath = filePathForTrack(track);
  if (!basePath) return "";
  const normalized = String(format || "mp3").toLowerCase();
  if (!["mp3", "wav"].includes(normalized)) return basePath;
  const currentExt = path.extname(basePath).toLowerCase();
  if (currentExt === `.${normalized}`) return basePath;
...
function stripeFormEncode(value, prefix = "") {
  ...
  Object.entries(value).forEach(([key, entry]) => {
    ...
    if (entry && typeof entry === "object" && !Array.isArray(entry)) {
      const nested = stripeFormEncode(entry, nextKey);
      nested.forEach((nestedValue, nestedKey) => params.append(nestedKey, nestedValue));
    } else {
      params.append(nextKey, String(entry ?? ""));
    }
  });
  return params;
}

async function createStripeCheckoutSession(request, body, user) {
  const secret = process.env.STRIPE_SECRET_KEY;
  if (!secret) throw new Error("Stripe checkout is not configured. Add STRIPE_SECRET_KEY in Vercel environment variables.");
  const planKey = sanitizeText(body.plan || "live-streaming-monthly");
  const plan = stripeCheckoutPlans[planKey];
  if (!plan) throw new Error("Unknown checkout plan.");
  const origin = requestOrigin(request);
  const productName = sanitizeText(body.productName, plan.name);
  const metadata = {
    userId: user.id,
    email: user.email,
    plan: planKey,
    entitlement: plan.entitlement,
    trackId: sanitizeText(body.trackId),
    albumId: sanitizeText(body.albumId || (plan.entitlement === "album" ? "jesus-christ-the-lamb-of-royal-mercies" : "")),
  ...
  if (plan.mode === "subscription") priceData.recurring = { interval: plan.interval || "month" };
...
async function handleStripeWebhook(request) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    const updated = grantPurchase(users, metadata.userId || session.client_reference_id, metadata);
...
export async function handleApi(request, pathParts) {
  ...
  await loadRemoteAlbumsIndex();
  await loadRemoteMediaIndex({ recoverOrphans: !(pathname === "/api/media" && request.method === "POST") });
  try {
    ...
    if (pathname === "/api/blob-upload" && request.method === "POST") {
      ...
      const result = await handleUpload({
        ...
        onBeforeGenerateToken: async () => {
          return {
            allowedContentTypes: [...allowedMediaTypes, ...allowedArtworkTypes],
    ...
    if (pathname === "/api/media" && request.method === "POST") {
      ...
      const googleDriveUrl = sanitizeText(body.googleDriveUrl);
      const drive = googleDriveUrl ? parseGoogleDriveUrl(googleDriveUrl) : null;
      const blobUrl = sanitizeText(body.blobUrl || body.url);
      const isBlobUpload = Boolean(blobUrl && blobUrlPattern.test(blobUrl));
      const uploadSources = [Boolean(parsed), Boolean(youtube), isBlobUpload, Boolean(drive)].filter(Boolean).length;
      if (googleDriveUrl && !drive) return json({ ok: false, error: "Paste a valid Google Drive file URL." }, 400);
      if (!uploadSources) return json({ ok: false, error: "Upload an audio/video file, provide a YouTube URL, or provide a Google Drive file URL." }, 400);
      if (uploadSources > 1) return json({ ok: false, error: "Choose only one upload source: file upload, Blob URL, YouTube URL, or Google Drive URL." }, 400);
      ...
      const artists = readJsonFile(artistsFile);
      const albums = readJsonFile(albumsFile);
      const selectedAlbum = albums.find((album) => album.id === sanitizeText(body.albumId));
      const selectedArtistId = sanitizeText(body.artistId || selectedAlbum?.artistId);
      const selectedArtist = artists.find((artist) => artist.id === selectedArtistId);
      const driveImport = drive ? await importGoogleDriveMedia(drive, body.title || "drive-media") : null;
      const cleanName = sanitizeFilename(body.fileName || body.originalName || driveImport?.fileName || body.title || "youtube-video");
      ...
      const mimeType = sanitizeText(body.mimeType || body.contentType || driveImport?.mimeType);
      ...
      const mediaItem = {
        ...
        artist: selectedArtist?.name || sanitizeText(body.artist),
        albumId: selectedAlbum?.id || sanitizeText(body.albumId),
        album: selectedAlbum?.title || sanitizeText(body.album),
        genre: sanitizeText(body.genre || selectedAlbum?.genre || selectedArtist?.genre),
        ...
        releaseStatus: sanitizeText(body.releaseStatus, "active"),
        isSnippet: body.isSnippet === "on" || body.isSnippet === true || String(body.kind || "").includes("snippet"),
        provider: youtube ? "youtube" : isBlobUpload || driveImport ? "blob" : "file",
        youtubeId: youtube?.videoId || "",
        embedUrl: youtube?.embedUrl || "",
        mimeType: youtube ? "text/youtube" : isBlobUpload || driveImport ? mimeType : parsed.mimeType,
        originalName: cleanName,
        storedName,
        size: parsed ? parsed.buffer.length : driveImport?.size || Number(body.size || 0),
        url: youtube ? youtube.url : isBlobUpload ? blobUrl : driveImport ? driveImport.url : `/media/${storedName}`,
        source: driveImport ? "google-drive" : "",
        sourceUrl: driveImport?.sourceUrl || "",
        sourceId: driveImport?.driveId || ""
      };
      const media = dedupeMediaByUrl(readJsonFile(mediaIndexFile)).filter((item) => item.url !== mediaItem.url);
      media.unshift(mediaItem);
      writeJsonFile(mediaIndexFile, media);
      await saveRemoteMediaRecord(mediaItem);
      await saveRemoteMediaIndex(media);
      return json({ ok: true, media: mediaItem }, 201);
    }
    if (pathname === "/api/media" && request.method === "DELETE") {
      if (!requireAdmin(request)) return json({ ok: false, error: "Admin access is required to delete media." }, 403);
      const mediaId = sanitizeText(new URL(request.url).searchParams.get("id"));
      if (!mediaId) return json({ ok: false, error: "Media id is required." }, 400);
      const media = dedupeMediaByUrl(readJsonFile(mediaIndexFile));
      const mediaItem = media.find((item) => item.id === mediaId);
      if (!mediaItem) return json({ ok: false, error: "Uploaded media was not found." }, 404);
      const next = media.filter((item) => item.id !== mediaId);
      if (mediaItem.provider === "blob" && blobUrlPattern.test(String(mediaItem.url || ""))) {
        if (canUseBlobMetadata()) await deleteBlob(mediaItem.url);
      } else if (mediaItem.storedName) {
        const filePath = path.join(mediaDir, path.basename(mediaItem.storedName));
        if (filePath.startsWith(mediaDir) && fs.existsSync(filePath)) fs.unlinkSync(filePath);
      }
      writeJsonFile(mediaIndexFile, next);
      if (canUseBlobMetadata()) {
        await deleteBlob(`${remoteMediaRecordsPrefix}${mediaItem.id}.json`).catch(() => {});
      }
      await saveRemoteMediaIndex(next);
      return json({ ok: true, deletedId: mediaId });
    }
    ...
    if (pathname === "/api/albums" && request.method === "GET") {
      ...
      return json({
        ok: true,
        albums: readJsonFile(albumsFile).map((album) => ({
          ...album,
          artist: album.artist || getArtistName(album.artistId, artists)
        }))
      });
    }
    if (pathname === "/api/albums" && request.method === "POST") {
      ...
      const album = {
        ...
        artwork: sanitizeText(body.artworkUrl || body.artwork, defaultAlbumArtwork),
        artworkName: sanitizeText(body.artworkName),
        artworkMimeType: sanitizeText(body.artworkMimeType),
        artworkSize: Number(body.artworkSize || 0),
        createdAt: new Date().toISOString()
      };
      ...
      await saveRemoteAlbumsIndex(albums);
    ...
    if (pathname === "/api/download" && request.method === "GET") {
      ...
      const trackId = sanitizeText(url.searchParams.get("trackId"));
      const format = sanitizeText(url.searchParams.get("format"), "mp3").toLowerCase();
      const track = findCatalogTrack(trackId);
      if (!track) return json({ ok: false, error: "Track not found." }, 404);
      if (!hasDownloadAccess(user, track)) return json({ ok: false, error: "Purchase this track, buy the album, or subscribe to live streaming to download media files." }, 403);
      if (track.provider === "blob" && blobUrlPattern.test(String(track.url || ""))) {
        return Response.redirect(track.url, 302);
      }
      const filePath = filePathForTrackFormat(track, format);
      if (!filePath || !fs.existsSync(filePath)) return json({ ok: false, error: `${format.toUpperCase()} download file is not available yet.` }, 404);
    ...
    if (pathname === "/api/checkout" && request.method === "POST") {
      const user = getUserFromSession(request);
      if (!user) return json({ ok: false, error: "Sign in before checkout." }, 401);
      const body = await request.json();
      const session = await createStripeCheckoutSession(request, body, user);
