...
function isCatalogDuplicate(item) {
  ...
  const isStaticBlackIndigoTrack = isBlackIndigo && ["odu mi o"].includes(title);
...
export async function handleApi(request, pathParts) {
  ...
  try {
    if (pathname === "/api/media" && request.method === "GET") {
      return json({ ok: true, media: readJsonFile(mediaIndexFile) });
    }
    if (pathname === "/api/media" && request.method === "POST") {
      ...
      if (!parsed && !youtube) return json({ ok: false, error: "Upload an audio/video file or provide a valid YouTube URL." }, 400);
      if (parsed && youtube) return json({ ok: false, error: "Choose either a file upload or a YouTube URL, not both." }, 400);
    ...
    if (pathname === "/api/signup" && request.method === "POST") {
      ...
      return json({ ok: true, user: { id: user.id, email: user.email, name: user.name, role: user.role }, token: session.token }, 201);
    }
    if (pathname === "/api/login" && request.method === "POST") {
      ...
      const session = createSession(user.id);
      return json({ ok: true, user: { id: user.id, email: user.email, name: user.name, role: isAdminUser(user) ? "admin" : "user" }, token: session.token });
    }
    ...
    if (pathname === "/api/me" && request.method === "GET") {
      ...
      return json({ ok: true, user: { id: user.id, email: user.email, name: user.name, role: isAdminUser(user) ? "admin" : "user" } });
    }
