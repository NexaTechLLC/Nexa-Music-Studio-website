...
function isRecoveredBlobOrphan(item) {
  return String(item.id || "").startsWith("blob-")
    && !item.albumId
    && !item.artistId
    && String(item.album || "") === "Uploaded Media"
    && String(item.artist || "") === "NEXAMusic™ Studios";
}
...
function uploadedTracks() {
  ...
  return dedupeMediaByUrl(readJsonFile(mediaIndexFile))
    .filter((item) => !isRecoveredBlobOrphan(item))
